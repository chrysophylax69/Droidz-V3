...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: droidz.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: droidzd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 5914 or testnet: 15914)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 5915 or testnet: 15915)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizedroidzamount=<n> " + _("Keep N Droidz anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Droidz is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Droidz is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Droidz version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Droidz server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Droidz") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Droidz to complete") << "\n";
    ...
    nAnonymizeDroidzAmount = GetArg("-anonymizedroidzamount", 0);
    if(nAnonymizeDroidzAmount > 999999) nAnonymizeDroidzAmount = 999999;
    if(nAnonymizeDroidzAmount < 2) nAnonymizeDroidzAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeDroidzAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Droidz Amount %d\n", nAnonymizeDroidzAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DRZ+1000 == (.1DRZ+100)*10
       10DRZ+10000 == (1DRZ+1000)*10
    */
